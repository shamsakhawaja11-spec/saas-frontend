...
const WorkspacesPage = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workspaces'], refetchType: 'all' });
      setShowModal(false);
      setName('');
      setDescription('');
      setError('');
      ...
      <div className="px-8 py-8">
        ...
        {!isLoading && workspaces.length === 0 && (
          <div className="border-2 border-dashed border-slate-700 rounded-xl p-14 text-center">
            ...
            <p className="text-slate-600 text-xs mt-1">Create your first workspace to get started</p>
            <button
              onClick={() => setShowModal(true)}
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-semibold text-white text-sm">{workspace.name}</h3>
                  </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
          <div className="bg-dark-800 border border-slate-700 rounded-2xl p-6 w-full max-w-md">
            <h2 className="text-lg font-bold text-white mb-5">Create Workspace</h2>
